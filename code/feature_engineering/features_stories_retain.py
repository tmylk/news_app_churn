...
def create_first_session_events():
    ...
    condition = """e."timestamp"  - """ + \
        str(FIRST_SESSION_HOURS) + """*60*60  <  f.first_seen_ts"""
...
def create_second_day_events():
    ...
    condition = """e."timestamp"  >  f.first_seen_ts + """ + \
        str(FIRST_SESSION_HOURS) + """*60*60"""
    return cut_events_by_time(view_name, condition)


def get_filter_clean_users_sql(table):
    '''
    Pnly use clean users in the analysis
    A clean user is a global condition, need all events of user's lifetime
    to decide.
    Looking at first and second day separately is not sufficient.
    Need to join with clean_user generated from the raw import of all events
    '''
    ame = "clean_users"
