...
def create_weeks_during_users_life():
    ...
    return create_materialized_view(view_name, '''
        SELECT distinct distinct_id, dates.week
        FROM dates
        JOIN first_last_seen fl
        ON
            dates.date >=
...
def create_user_one_city_country():
    ...
    SELECT
        ...
        FIRST_VALUE("$city") OVER (PARTITION BY distinct_id ) AS "$city",
        FIRST_VALUE("mp_country_code") OVER
...
#     return create_view(view_name,"""
#     SELECT distinct_id, AVG(day_total::real) as day_avg,
# SUM(day_total) as total, SUM(CASE WHEN day_total = 0 THEN 0 ELSE 1 END)
# as day_active  FROM day_totals
...
#     return create_view(view_name,"""
#     SELECT distinct_id, AVG(week_total::real) as week_avg,
# SUM(week_total) as total, SUM(CASE WHEN week_total = 0 THEN 0 ELSE 1 END)
# as week_active     FROM week_totals
#     WHERE event_type='story completion'
#     GROUP BY distinct_id
#     """)
