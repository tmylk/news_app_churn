...
class event_features(object):
    ...
    def create_view(self, view_name, sql):
        return helpers.create_materialized_view(self.event_table_alias + view_name,sql)
    ...
    #TODO: maybe exclude weeks 3 and 9 from analysis as they are shorter than normal?
    ...
    def create_all_week_event_stats(self):
        ...
        for event_type in df_events.event_type.values:
            ...
            sql = """SELECT distinct_id, AVG(week_total::real) as week_avg, SUM(week_total) as total, SUM(CASE WHEN week_total = 0 THEN 0 ELSE 1 END) as week_active
                     FROM """+self.event_table_alias + """week_totals
                     WHERE event_type='$event_type$' GROUP BY distinct_id;
                     """
    ...
    def create_all_day_event_stats(self):
        ...
        for event_type in df_events.event_type.values:
            ...
            sql = """SELECT distinct_id, AVG(day_total::real) as day_avg, SUM(day_total) as total, SUM(CASE WHEN day_total = 0 THEN 0 ELSE 1 END) as day_active
                     FROM """+self.event_table_alias + """day_totals
    ...
    def create_all_agg_day_event_stats(self):
        ...
        for event_type in df_events.event_type.values:
            ...
            sql = """SELECT d.week, d.date, AVG(day_total::real) as agg_day_avg, SUM(day_total::real) as agg_day_total, SUM(CASE WHEN day_total = 0 THEN 0 ELSE 1 END) as agg_day_unique
    ...
    # how often a user read X% of the article and how long it took them to get there
    ...
    """
        ...
        RIGHT JOIN d ON 
         ...
         GROUP BY d.week, d.date, d.distinct_id, d.event_type, r."story completion", r."story id" ;  """)
    ...
    def create_time_to_x_agg_day_stats(self):
        ...
        FROM 
        (   SELECT * FROM """+self.event_table_alias + """time_to_x_day_totals  WHERE "story completion"=$level$
...
##### Collecting features together #####
    def get_all_day_event_stats_tables(self,first, last):
        ...
        engine= create_engine(conn_str)
        df_events = pd.read_sql("SELECT * FROM event_types ORDER BY event_type ", engine)
    ...
    def get_all_story_stats_tables(self,first, last):
        ...
        df_stories = pd.read_sql("""SELECT * FROM stories ORDER BY "story id" """, engine)
        df_stories = df_stories[first:last]
        
        for story_id in df_stories["story id"].values:        
            column_aliases = {}
            sql_story_id=helpers.clean_string_for_column_name(story_id)
            view_name = self.event_table_alias + "story_" +sql_story_id+"_day_stats"
            original_columns = ["agg_day_count_avg" ]#,"agg_day_time_avg", "agg_day_count", "agg_day_count_avg"]
            for col in original_columns:
    ...
    def get_all_time_stats_tables(self):
        ...
        for level in self.levels:     
            column_aliases = {}
            my_view_name = view_name.replace('$level$', level)
            original_columns = ["agg_day_time_total","agg_day_time_avg", "agg_day_count", "agg_day_count_avg"   ]
            for col in original_columns:
    ...
    def get_all_static_features(self):
       ...
       tables["user_one_city_country"]= {"\"$city\"":"\"city\"","mp_country_code":"mp_country_code"}
       tables["user_screen"]=  {"max_width":"max_width", "max_height":"max_height"}
     
       return tables

    def get_all_first_last_event_tables(self):
        tables = {}
        tables["first_events"]= {"first_event_type":"first_event_type","first_event_new_user":"first_event_new_user","first_event_result":"first_event_result"}
        tables["last_events"]= {"last_event_type":"last_event_type","last_event_new_user":"last_event_new_user","last_event_result":"last_event_result"}
        return tables
    ...
    def aggregate_all_features(self,tables, use_star = False):        
        ...
        for tbl in tables.keys()[1:]:
            end += "\""+ tables.keys()[0] +"\""+ ".distinct_id = \"" +tbl +"\".distinct_id AND "
        end = end[:-4]
    ...
    # had to split into 2 chunks as the query was too long for redshift to process
    # TODO: do the same thing in Python in dictVectorizer and pickle .
    #       Feature activation is low, so the data is sparse. (Didn't expect it when started the project)
    ...
    # TODO: try the same thing in Python in dictVectorizer and pickle. See above
    ...
    def create_users_all_features(self):
        ...
        tables= { "users_all_static_features":"", self.event_table_alias + "users_all_time_features":"", self.event_table_alias + "users_all_event_features_1":"", "users_all_event_features_2":"", } 
        return self.create_view(view_name,self.aggregate_all_features(tables, use_star = True))

    def create_users_all_stories(self):
        view_name = "users_all_stories"
        tables= { "users_all_static_features":"", self.event_table_alias + "users_all_story_features_0":"",self.event_table_alias + "users_all_story_features_1":"", self.event_table_alias + "users_all_story_features_2":"", self.event_table_alias + "users_all_story_features_3":"", self.event_table_alias + "users_all_story_features_4":"", self.event_table_alias + "users_all_story_features_5":"", self.event_table_alias + "users_all_story_features_6":"" } 
        return self.create_view(view_name,self.aggregate_all_features(tables, use_star = True))
