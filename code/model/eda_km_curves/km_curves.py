...
def add_date_day(df, column):
    ...
    df[column + '_day'] = \
        df[column + '_date'].apply(lambda dt:
...
def create_stay_col(df, first_seen_cut_off_date, last_seen_cut_off_date):
    df_stay = df[
        df.first_seen_ts_date < pd.to_datetime(first_seen_cut_off_date)]

    df_stay['stay'] = (
        df_stay.last_seen_ts_date >= pd.to_datetime(last_seen_cut_off_date))
    return df_stay
...
def get_data_from_csv():
    '''
    once data is cleaned, can save and re-use from csv
    '''
...
def plot_KM(df, conditions, label_texts, colors=['b'], first_days=0,
            cut_off_date=pd.to_datetime('2015-02-28'),
            first_seen_date=pd.to_datetime('2015-01-15'), ax=None):
    ...
    for (condition, label_text, color) in zip(conditions, label_texts, colors):
        ...
        selected = data[
            (condition) & (
                df.age_days > first_days) & (
                df.first_seen_ts_date > first_seen_date)]
        kmf.fit(selected['ll_duration'],
                selected['ll_event'],
                label=label_text + ', size:' + str(len(selected)),
                app=t)
...
if __name__ == '__main__':
    ...
    # the interesting part here is that they are almost the same, see pdf
    # presentation
